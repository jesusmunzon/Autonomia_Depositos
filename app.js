// Real demand curves over 24 hours (l/s) matching Excel average magnitudes
        ...
        function updateEntronqueCharts() {
            ...
            const levelOptions = {
                ...
                markers: { size: 0, strokeWidth: 2, hover: { size: 7, sizeOffset: 3 } },
                dataLabels: { enabled: false },
                xaxis: { categories: labels, tickAmount: isPrintMode() ? 6 : 10, tickPlacement: 'between', labels: { rotate: 0, rotateAlways: false, hideOverlappingLabels: true, trim: false, formatter: value => value, style: { fontSize: isPrintMode() ? '6px' : '10px', colors: '#64748b' }, offsetY: isPrintMode() ? 10 : 0 }, tooltip: { enabled: false } },
                yaxis: { min: Math.max(0, Math.floor(Math.min(minValue, minLimit) - 0.5)), max: Math.ceil(maxValue + 0.5), labels: { formatter: v => `${v.toFixed(2)} m`, style: { colors: '#64748b' } } },
                tooltip: { shared: false, intersect: false, followCursor: true, x: { show: true }, y: { formatter: v => `${v.toFixed(2)} m` } },
                legend: {
                    show: true,
