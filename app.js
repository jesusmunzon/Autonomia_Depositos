// Real demand curves over 24 hours (l/s) matching Excel average magnitudes
            ...
            reader.onload = function(e) {
                try {
                    ...
                    pendingExcelImport = {
                        ...
                        entrada: records.map(record => record.entrada),
                        salida1: records.map(record => record.salida1),
                        burguillos: records.map(record => record.burguillos)
                    };
                    pendingExcelImport.startDate.setMilliseconds(0);
        ...
        function saveModalChanges() {
            ...
            if (pendingExcelImport && pendingExcelImport.target === activeModalTarget) {
                ...
                targetObj.timestamps = Array.from({ length: Math.min(168, targetObj.entradaProfile.length) }, (_, i) => new Date(targetObj.startDate.getTime() + i * 3600000));
            ...
            const newAvg = targetObj.entradaProfile.reduce((a, b) => a + b, 0) / targetObj.entradaProfile.length;
